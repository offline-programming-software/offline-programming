...
MainWindow::MainWindow(QWidget* parent) : SARibbonMainWindow(parent)
{
	...
	SARibbonCategory* sence = bar->addCategoryPage("  编程仿真  ");
		...
		SARibbonPannel*  path = sence->addPannel("轨迹规划");
			QAction* action68 = path->addAction("工作空间定义", QIcon(":/image/resource/robot_space.svg"), QToolButton::InstantPopup);
				connect(action68, SIGNAL(triggered()), this, SLOT(on_define_space()));//工作空间定义
			...
			QAction* action70 = path->addAction("站位计算", QIcon(":/image/resource/100.png"), QToolButton::InstantPopup);
				connect(action70, SIGNAL(triggered()), this, SLOT(on_pos_cal()));//喷涂区域划分
			QAction* action65 = path->addAction("AGV路径优化", QIcon(":/image/resource/icons8-agv-32.png"), QToolButton::InstantPopup);
				connect(action65, SIGNAL(triggered()), this, SLOT(on_AGV_path()));//AGV路径优化
		...
		SARibbonPannel*  output= sence->addPannel("输出");
			QAction* action15 = output->addAction("后置", QIcon(":/image/resource/26.png"), QToolButton::InstantPopup);
	...
	//添加**定义机构**菜单//
	//******************//
	SARibbonCategory* definition = bar->addCategoryPage(" 场景搭建 ");
		//|机构|
		SARibbonPannel* def = definition->addPannel("机构");
		...
		SARibbonPannel* tool = definition->addPannel("工具");
			...
			QAction* action47 = part->addAction("定义AGV", QIcon(":/image/resource/icons8-agv-32.png"), QToolButton::InstantPopup);
					connect(action47, SIGNAL(triggered()), this, SLOT(on_path_sort()));//喷刷管理

			//|模型导入|
			SARibbonPannel* impoertModel = definition->addPannel("模型导入");
			...
			//|添加设置|
			SARibbonPannel* set = definition->addPannel("设置");
