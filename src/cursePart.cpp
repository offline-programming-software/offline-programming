...
void cursePart::on_calculate_workspace()
{
	...
	thetaStr.remove(QRegExp(R"([°??°?])"));  // 移除常见的角度符号
	...
	std::vector<std::wstring>>&pickupMap)
{
	...
	for (const auto& pair : pickupMap) {
		...
		double* dPosition = new double[static_cast<size_t>(lCount) * 3]{};
		HRESULT hrVertex = m_ptrKit->PQAPIGetWorkPartVertex(ulPartID, 0, lCount, dPosition);
		if (FAILED(hrVertex)) {
			delete[] dPosition;
			continue;
		}
		...
		for (LONG i = 0; i < lCount; ++i) {
			const double vx = dPosition[3 * i];
			const double vy = dPosition[3 * i + 1];
			const double vz = dPosition[3 * i + 2];
