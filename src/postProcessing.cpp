...
bool postProcessing::generateRobotPostFiles(int robotNodeId)
{
	...
	while (it.hasNext()) {
		...
		// 写入轨迹文件
		QString trajectoryFilePath = groupFolderPath + "/" + groupFolderName + "_trajectory.JBQ";
	...
	const QString& pathGroupName) const
{
	...
	QString outputFilename = QString("%1_%2.JBQ").arg(robotName).arg(pathGroupName);
	...
	std::vector<RawPointData> rawPoints;
	std::vector<double> rawTimes;
	std::vector<double> pathStartTimes;
			...
			for (ULONG pathIndex = 0; pathIndex < pathCount; ++pathIndex) {
				...
				for (int ptIndex = 0; ptIndex < pointCount; ++ptIndex) {
                    ...
                    if (jointDataVariant.parray && jointDataVariant.parray->cDims == 1) {
                        ...
                        if (SUCCEEDED(SafeArrayAccessData(jointArray, (void**)&jointData)) && jointData) {
                            ...
                            for (ULONG j = 0; j < jointCount; ++j) {
                                // 将弧度转换为角度
                                double angleInRadians = jointData[j];
                                double angleInDegrees = angleInRadians * 180.0 / 3.14;  // M_PI 是 π 的定义
                                rawPoint.robotJoints.push_back(angleInDegrees);
                            }
                    ...
                    if (externalId != 0 && externalJointsVariant.parray && externalJointsVariant.parray->cDims == 1) {
						...
						if (SUCCEEDED(SafeArrayAccessData(extArray, (void**)&extData)) && extData) {
							...
							for (ULONG e = 0; e < extCount; ++e) {
                             rawPoint.externalAxes.push_back(extData[e]);
							}
                    ...
                    if (SUCCEEDED(hrEvent)) {
                         ...
                         rawPoint.event.hasEvent = true;
							rawPoint.event.name = CComBSTR(eventName.toStdWString().c_str());
							rawPoint.event.content = CComBSTR(eventContent.toStdWString().c_str());
							rawPoint.event.position = CComBSTR(pointForward != 0 ? L"before" : L"after");
							}
						}
                    ...
                    if (!rawPoint.robotJoints.empty()) {
						...
						if (rawPoint.isPathStart) {
							pathStartTimes.push_back(timeValue);
						}
					}
	...
	auto appendRawPoints = [&]() {
		for (size_t i = 0; i < rawPoints.size(); ++i) {
			TrajectoryPointInfo pointInfo;
			pointInfo.pointIndex = globalPointIndex++;
			pointInfo.motionType = rawPoints[i].isPathStart ? "MOVJ" : "MOVD";
			pointInfo.motionPercentage = rawPoints[i].speedPercent;
			pointInfo.velocity = rawPoints[i].velocity;
			pointInfo.robotJoints = rawPoints[i].robotJoints;
			pointInfo.externalAxes = rawPoints[i].externalAxes;
			pointInfo.event = rawPoints[i].event;
			generator.addTrajectoryPoint(pointInfo);
		}
	};

	auto findTimeMatchIndex = [](const std::vector<double>& times, double value, double eps) -> int {
		for (size_t i = 0; i < times.size(); ++i) {
			if (std::fabs(times[i] - value) <= eps) {
	...
	auto computeDerivatives = [](const std::vector<std::vector<double>>& positions,
		...
		accelerations.assign(count, std::vector<double>(axes, 0.0));
	...
	bool interpolationSucceeded = false;
	if (rawPoints.size() >= 2) {
		try {
			...
			if (!positions.empty() && positions.size() == timeSteps.size()) {
				...
				auto getRawIndexForTime = [&](double timeValue) -> size_t {
					auto it = std::upper_bound(rawTimes.begin(), rawTimes.end(), timeValue);
					if (it == rawTimes.begin()) {
						return 0;
					}
					return static_cast<size_t>(it - rawTimes.begin() - 1);
				};
				...
				for (size_t i = 0; i < positions.size(); ++i) {
					...
					if (i < railPositionsInterp.size()) {
						pointInfo.externalAxes = railPositionsInterp[i];
					}
					if (eventIndex >= 0) {
						pointInfo.event = rawPoints[eventIndex].event;
					}
					generator.addTrajectoryPoint(pointInfo);
				}

				interpolationSucceeded = true;
			}
		}
